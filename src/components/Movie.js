...
function Movie({ movie }) {
    const [like, setSavedLike] = useState(false);
    const [saved, setSavedFavorite] = useState(false);
    const { user } = UserAuth();
    const movieID = doc(db, 'users', `${user?.email}`);

    const saveFavoriteShow = async () => {
        if (user?.email) {
            setSavedLike(!like);
    ...
    return (
        ...
        items-center h-full text-center">{movie?.title}</p>
                <p onClick={saveFavoriteShow}>
